def filter_by_state(operations: list[dict[str, str]], state: str = "EXECUTED") -> list[dict[str, str]]:
    """Функция принимает список операций и возвращает список операций отфильтрованный по параметру 'state'.
    Данные по каждой операции представлены в виде словаря."""
    filtered_operations: list[dict[str, str]] = [operation for operation in operations if operation["state"] == state]
