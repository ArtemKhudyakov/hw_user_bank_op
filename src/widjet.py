def mask_account_card(input_data: str) -> str:
    """Функция принимает номер счет в формате "Счет 73654108430135874305" или номер карты в формате
    ...
    if len(splited_input_data[-1]) == 20:
        ...
        splited_masked_acc: list = [i for i in splited_input_data if not i.isdigit()]
        splited_masked_acc.append(masked_acc)
        return " ".join(splited_masked_acc)

    elif len(splited_input_data[-1]) == 16:
        masked_card: str = src.masks.get_mask_card_number(splited_input_data[-1])
        splited_masked_card: list = [i for i in splited_input_data if not i.isdigit()]
...
def get_date(date: str) -> str:
    """Функция принимает дату в формате "2024-03-11T02:26:18.671407" и возвращает дату в формате "ДД.ММ.ГГГГ"
    ("11.03.2024")."""
    ...
    transformated_date = f"{date_as_dict['day']}.{date_as_dict['month']}.{date_as_dict['year']}"
